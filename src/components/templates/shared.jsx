...
export function buildTemplateData(cv) {
  const personal = cv.personal ?? {};

  return {
    name: [personal.firstName, personal.lastName].filter(Boolean).join(" ").trim() || "Vorname Nachname",
    ...
    education: (cv.education ?? []).map(item => ({
      ...
      note: item.grade ? `Note ${item.grade}` : "",
    })),
...
export function SkillBar({ name, level, accent, dark = false }) {
...
export function SkillRadar({ skills, accent, dark = false }) {
  ...
  return (
    <svg style={{ maxWidth: 160, display: "block", margin: "0 auto" }} viewBox="0 0 160 160" width="100%">
...
export function SkillsBlock({ skills, display, accent, dark = false }) {
  ...
  if (display === "bars") {
    return (
      <div>
        {skills.map(skill => (
          <SkillBar key={skill.id} accent={accent} dark={dark} level={skill.level} name={skill.name} />
        ))}
      </div>
    );
  }
...
export function SidebarLabel({ children }) {
  return (
    <div style={{ color: "rgba(255,255,255,0.42)", fontSize: 9, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: 10, fontFamily: FONT_EDITOR }}>
      {children}
    </div>
  );
}
