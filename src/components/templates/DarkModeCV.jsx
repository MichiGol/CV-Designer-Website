import { normalizePhotoConfig, PhotoFrame, SkillsBlock } from "./shared.jsx";
...
export function DarkModeCV({ cv, st, displayFont, bodyFont, pageMode = "preview" }) {
  ...
  return (
    <div style={{ background: bg, ...(isExportPage ? { minHeight: "100%", height: "100%", boxSizing: "border-box", display: "flex", flexDirection: "column" } : { minHeight: 900 }) }}>
      <div style={{ background: `linear-gradient(135deg,${surface} 0%,${bg} 100%)`, borderBottom: `1px solid ${border}`, padding: "40px 44px 32px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 28, position: "relative", zIndex: 1 }}>
          ...
          />
          <div style={{ flex: 1 }}>
            <h1 style={{ color: textPrimary, margin: "0 0 4px", fontSize: size * 2.3, fontWeight: 800, fontFamily: displayFont, letterSpacing: "-0.02em", lineHeight: 1.1 }}>{personal.firstName || "Vorname"} {personal.lastName || "Nachname"}</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", ...(isExportPage ? { flex: 1, minHeight: 0 } : {}) }}>
        <div style={{ background: surface, borderRight: `1px solid ${border}`, padding: "8px 20px 30px" }}>
          {cv.skills.length ? <><DS>Fähigkeiten</DS><SkillsBlock accent={accent} dark display={skillDisplay} skills={cv.skills} /></> : null}
          {cv.languages.length ? <><DS>Sprachen</DS>{cv.languages.map(language => <div key={language.id} style={{ marginBottom: 10 }}><div style={{ fontWeight: 700, fontSize: size * 0.88, color: textPrimary, fontFamily: bodyFont }}>{language.name}</div><div style={{ color: textMuted, fontSize: size * 0.78, marginTop: 2, fontFamily: bodyFont }}>{language.level}</div></div>)}</> : null}
          {cv.hobbies.length ? <><DS>Interessen</DS><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{cv.hobbies.map((hobby, index) => <span key={index} style={{ background: `${accent}18`, color: accent, padding: "3px 9px", borderRadius: 20, fontSize: size * 0.78, fontWeight: 600, fontFamily: bodyFont, border: `1px solid ${accent}30` }}>{hobby}</span>)}</div></> : null}
        </div>
        <div style={{ padding: "8px 36px 40px", background: bg }}>
          {personal.summary ? <><DS>Profil</DS><p style={{ lineHeight: 1.75, color: textSecondary, margin: "0 0 8px", fontSize: size * 0.92, fontFamily: bodyFont, background: `${accent}0a`, borderLeft: `3px solid ${accent}`, padding: "10px 14px", borderRadius: "0 6px 6px 0" }}>{personal.summary}</p></> : null}
          {cv.experience.length ? <><DS>Berufserfahrung</DS><div style={{ borderLeft: `2px solid ${accent}30` }}>{cv.experience.map(experience => <DT key={experience.id} body={experience.description} period={experience.start && experience.end ? `${experience.start} – ${experience.end}` : ""} sub={experience.company} title={experience.role} />)}</div></> : null}
          {cv.education.length ? <><DS>Ausbildung</DS><div style={{ borderLeft: `2px solid ${accent}30` }}>{cv.education.map(education => <DT key={education.id} body={education.description} extra={education.grade ? `Ø ${education.grade}` : ""} period={education.start && education.end ? `${education.start} – ${education.end}` : ""} sub={education.institution} title={education.degree} />)}</div></> : null}
        </div>
      </div>
    </div>
  );
