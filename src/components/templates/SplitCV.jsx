import { normalizePhotoConfig, PhotoFrame, SecTitle, SidebarLabel, SkillsBlock, TLItem } from "./shared.jsx";
...
export function SplitCV({ cv, st, displayFont, bodyFont, pageMode = "preview" }) {
  ...
  return (
    <div style={{ display: "flex", ...(isExportPage ? { minHeight: "100%", height: "100%", boxSizing: "border-box" } : { minHeight: 900 }) }}>
      <div style={{ width: "32%", flexShrink: 0, background: accent, padding: "40px 22px 40px", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
        ...
        {cv.skills.length ? <><SidebarLabel>Fähigkeiten</SidebarLabel><div style={{ marginBottom: 20 }}><SkillsBlock accent={accent} dark display={skillDisplay} skills={cv.skills} /></div></> : null}
        {cv.languages.length ? <><SidebarLabel>Sprachen</SidebarLabel><div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>{cv.languages.map(language => <div key={language.id}><div style={{ color: "rgba(255,255,255,0.9)", fontSize: size * 0.84, fontWeight: 600, fontFamily: bodyFont }}>{language.name}</div><div style={{ color: "rgba(255,255,255,0.5)", fontSize: size * 0.75, fontFamily: bodyFont }}>{language.level}</div></div>)}</div></> : null}
        {cv.hobbies.length ? <><SidebarLabel>Interessen</SidebarLabel><div style={{ color: "rgba(255,255,255,0.65)", fontSize: size * 0.79, fontFamily: bodyFont, lineHeight: 1.8 }}>{cv.hobbies.join("  ·  ")}</div></> : null}
      </div>
      <div style={{ flex: 1, padding: "40px 34px 40px", background: "white" }}>
        {personal.summary ? <><SecTitle accent={accent} bodyFont={bodyFont}>Profil</SecTitle><p style={{ color: "#475569", lineHeight: 1.78, fontSize: size * 0.92, fontFamily: bodyFont, margin: "0 0 24px" }}>{personal.summary}</p></> : null}
