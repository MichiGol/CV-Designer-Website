import { normalizePhotoConfig, PhotoFrame, SecTitle, SkillsBlock, TLItem, useResumeTemplateCopy } from "./shared.jsx";

export function HeaderCV({ cv, st, displayFont, bodyFont, pageMode = "preview" }) {
  const copy = useResumeTemplateCopy();
  const [fallbackFirstName, ...fallbackLastNameParts] = copy.nameFallback.split(" ");
  ...
  return (
    <div style={isExportPage ? { boxSizing: "border-box" } : undefined}>
        ...
        <div className="cv-preview-hero" style={{ display: "flex", alignItems: "center", gap: 28, padding: "36px 44px 24px", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ flex: 1 }}>
            <h1 style={{ color: "white", margin: "0 0 4px", fontSize: size * 2.3, fontWeight: 800, fontFamily: displayFont, letterSpacing: "-0.02em", lineHeight: 1.1 }}>
              {personal.firstName || fallbackFirstName} {personal.lastName || fallbackLastName}
            </h1>
      ...
      <div className="cv-preview-columns" style={{ display: "grid", gridTemplateColumns: sidebarSide === "left" ? "200px 1fr" : "1fr 200px", ...(isExportPage ? { alignItems: "start" } : { minHeight: 600 }) }}>
        <div className="cv-preview-sidebar" style={{ order: sidebarSide === "left" ? 1 : 2, background: "#f8fafc", borderLeft: sidebarSide === "right" ? "1px solid #e2e8f0" : "none", borderRight: sidebarSide === "left" ? "1px solid #e2e8f0" : "none", padding: "8px 20px 30px" }}>
          {cv.skills.length ? <><SecTitle accent={accent} bodyFont={bodyFont}>{copy.skills}</SecTitle><SkillsBlock accent={accent} display={skillDisplay} skills={cv.skills} /></> : null}
          {cv.languages.length ? <><SecTitle accent={accent} bodyFont={bodyFont}>{copy.languages}</SecTitle>{cv.languages.map(language => <div key={language.id} style={{ marginBottom: 10 }}><div style={{ fontWeight: 700, fontSize: size * 0.88, color: "#334155", fontFamily: bodyFont }}>{language.name}</div><div style={{ color: "#94a3b8", fontSize: size * 0.78, marginTop: 2, fontFamily: bodyFont }}>{language.level}</div></div>)}</> : null}
          {cv.hobbies.length ? <><SecTitle accent={accent} bodyFont={bodyFont}>{copy.interests}</SecTitle><div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>{cv.hobbies.map((hobby, index) => <span key={index} style={{ background: `${accent}12`, color: accent, padding: "3px 9px", borderRadius: 20, fontSize: size * 0.78, fontWeight: 600, fontFamily: bodyFont, border: `1px solid ${accent}25` }}>{hobby}</span>)}</div></> : null}
        </div>
        <div className="cv-preview-main" style={{ order: sidebarSide === "left" ? 2 : 1, padding: "8px 32px 36px" }}>
          {personal.summary ? <><SecTitle accent={accent} bodyFont={bodyFont}>{copy.profile}</SecTitle><p style={{ lineHeight: 1.75, color: "#334155", margin: "0 0 8px", fontSize: size * 0.92, fontFamily: bodyFont, background: `${accent}08`, borderLeft: `3px solid ${accent}`, padding: "10px 14px", borderRadius: "0 6px 6px 0" }}>{personal.summary}</p></> : null}
