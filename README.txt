...
Overview
--------
CV Designer is a React + Vite application for building resumes in two different ways:
...
3. `src/providers/AppProviders.jsx`
   Global providers: MUI theme and autosave context.
...
- `AppProviders.jsx`
  Wraps the app with `ThemeProvider`, `CssBaseline`, and `AutosaveProvider`.
...
- `TemplateEditor.jsx`
  Main structured editor shell.
  Handles photo upload, JSON import, PDF export, and transfer to canvas.

- `CanvasEditor.jsx`
  Main freeform editor shell.
  Owns toolbar, export surface, layers panel, canvas surface, and properties panel.
...
- `appShellConfig.js`
  Navigation labels and shared shell copy.
...
- `layoutConfig.js`
  Template registry, layout groups, skill display options, font pairs, and canvas element metadata.
...
- `useCVEditor.js`
  Older structured-editor hook. It is not the active route path anymore.
  The current template editor uses `templateEditorStore.js` instead.

`src/store/`

- `templateEditorStore.js`
  Central state for the template editor.
  If you need to change structured editor data flow, this is usually the first file to inspect.
...
- `templateToCanvas.js`
  Converts the rendered template DOM into freeform canvas elements.
  This is the bridge between the structured editor and the canvas editor.
...
- `CV-Designer-Sicherung/`
  Backup or older copy in the workspace.
  The active source code for the app described in this README lives under `src/`.
